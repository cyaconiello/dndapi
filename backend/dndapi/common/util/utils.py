...
from .choices import character_generation_stat_preference

def get_character_attribute_preference(data) -> str:
    preference = 'random'
    if 'preference' in data:
        if character_generation_stat_preference[data['preference']]:
            preference = character_generation_stat_preference[data['preference']]
    return preference
...
def get_character_stat_block_based_on_preference(
    ...
    weighted = False
    if (
        preference == character_generation_stat_preference['race_weighted_perferred'] or
        preference == character_generation_stat_preference['race_weighted_balanced']
    ):
        if preference == character_generation_stat_preference['race_weighted_perferred']:
            weighted = True
        stats = sort_stats_by_racial_strengths(race, stat_block, stats, weighted)
    elif (
        character_class and
        (preference == character_generation_stat_preference['class_weighted_perferred'] or
        preference == character_generation_stat_preference['class_weighted_balanced'])
        ):
        if preference == character_generation_stat_preference['class_weighted_perferred']:
            weighted = True
...
def random_assorted_stats(stat_block:list[int], stats: dict[str, int]) -> dict[str, int]:
